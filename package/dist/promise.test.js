...
test('concurrent transaction() calls must not interleave', async () => {
    ...
    const insertMany = db.transactionAsync(async (txn, tag, fail) => {
        for (let i = 0; i < 10; i++) {
    ...
    expect(aborted.reason.message).toBe('abort transaction');
...
test('transactionAsync() rejects callbacks that do not declare the handle', async () => {
    ...
    expect(() => db.transactionAsync((async (...args) => { }))).toThrow(/Transaction handle/);
});
